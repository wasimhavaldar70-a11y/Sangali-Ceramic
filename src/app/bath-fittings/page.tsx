...
export default async function BathFittingsPage() {
  ...
  const products = await dbService.getProducts(undefined, undefined, 'cat-bath');
  ...
  return (
    <div className="pt-32 pb-20 bg-dark-black max-w-7xl mx-auto px-6 md:px-12 min-h-screen">
      ...
      <div className="mt-20">
        <h2 className="text-2xl font-display font-bold text-white mb-2">All Products in Bath Fittings</h2>
        <p className="text-white/60 text-sm mb-8">
          Showing {products.length} {products.length === 1 ? 'product' : 'products'} for Bath Fittings
        </p>
        ...
        {products.length === 0 ? (
          ...
          <CategoryClient products={products} categoryName="Bath Fittings" />
        )}
      </div>
